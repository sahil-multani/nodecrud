...
router.post("/uploadImage", upload2.single("file"), async (req, res) => {
  ...
  let sender_id = channel_id+Date.now()
  ...
  async function moveFile(fileLocation) {
    ...
    let fullurl = `/image/${sender_id}/${channel_id}/${newname}`;
...
router.get('/:id/:channel_id/:file', (req, res) => {
	const { channel_id,  id, file } = req.params;
	let pathToFile = `C:\\app_uploads\\${id}\\${channel_id}\\${file}`;

	if (fs.existsSync(pathToFile)) res.sendFile(path.resolve(pathToFile));
